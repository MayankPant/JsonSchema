...
class JsonSchemaValidator():
    ...
    def validate(self, schema, user_input):
        try:
            ...
            logger.debug("JSON SCHEMA VALIDATION UNSUCCESSFUL")
            return False
    ...
    def validate_jsonschema(self, schema):
        ...
        """
        logger.debug(f"Recieved schema for validation: {schema}\n\n\n\n")
        try:
            schema = json.loads(schema)
...
def otp_generator(length: int = OTP_LENGTH, expiry_time: int = EXPIRY_TIME, user_email: str = None):
    try:
        ...
        logger.debug(f"OTP Details: {otp}")
        ...
        if response.status_code == 200:
            ...
            logger.debug(f"Failed to send email.  {response.content}")
    except Exception as e:
