...
async function submitForm(){
    ...
    const fileHash = await generateFileHash(document.getElementById('file_input').files[0]);
    console.log("Converted file hash of file: ", fileHash);
    formData.append('profile_picture_file_hash', fileHash);

    // remove the profile picture itself as its no longer needed
...
async function validate(){
        console.log("Entered the form validation method in signup.js");
        event.preventDefault();
        if(validateForm([document.getElementById('username'), document.getElementById('email'), document.getElementById('password'), document.getElementById('confirm_password'), ,document.getElementById('file_input')])){
...
export async function uploadImage(file) {
    ...
    try {
        ...
        const csrf_token = await getCookie("csrftoken");
        // gets the signature to confirm whether the requested server has the access for upload
